...
def validation_errors(rows: list[dict[str, float | None]]) -> list[str]:
    ...
    for key in ("mean_improvement_pct", "population_std_pct", "median_improvement_pct"):
...
def main() -> int:
    ...
    result = {
        ...
        "status": "PASS" if not errors and control["rejected"] else "FAIL",
        "source": {
    ...
    print(
        ...
        "mean={mean:.6f} std={std:.6f} median={median:.6f} control_rejected={control}".format(
            ...
            std=summary["population_std_pct"],
